...
The pipe operator ` %>% `, or function, is originally a part of `magrittr` package. It is automatically loaded when we load the tidyverse packages. 

The native `R` pipe operator is ` |> `, which is built-in when you install the latest version of `R`. ` %>% ` requires you to activate `{magrittr}` or `{tidyverse}` packages. Either one can be used to streamline the code. 

The pipe operator allows computations to run from left to right in a smooth and easily readable manner. For example 
```{r}
...
**Example**: Use `select()` and `filter()` together using the pipe operator to extract data of Brazil and Turkey after 2000. 
```{r}
gapminder |>  
  filter(country %in% c("Turkey","Brazil"), year>2000) |> 
...
**Example**: Create the natural logarithm of GDP per capita:   
```{r}
mutate(gapminder, lgdppc = log(gdpPercap)) |> head(5)
```

**Example**: In the Gapminder data set, there is GDP per capita and population but no GDP. Create the GDP variable. 
```{r}
...
We  `dplyr::summarize()` or `dplyr::summarise()` to compute summary statistics of our variables. By grouping variables using `group_by()` we can conveniently compute group summaries. 
...
**Example**: Find the average life expectancy and GDP per capita according to continents in 2007. 
```{r}
gapminder |> 
  filter(year==2007) |>
  group_by(continent) |>
...
```{r}
# create a new tibble called europe
europe <- gapminder |> 
  filter(continent=="Europe") |>
  select(year, country, lifeExp) 
# first 10 observations in europe in 1952 
europe |> 
  arrange(year) |>  
  head(10)
``` 
...
For more details, visit <https://r4ds.had.co.nz/tidy-data.html#tidy-data-1>.

## `pivot_longer()`: Convert into long form
...
```{r}
pivot_longer(table4a,            # data set
             c(`1999`, `2000`),  # which columns
             names_to = "year",  # column names will become cell entries 
             values_to = "cases" # values go into new variable cases
             )
```

Compare this to the original table: 
```{r}
table4a
```


The same operation using the pipe operator:  
```{r}
table4a |> 
...
Convert this into long form creating a new variable population: 
```{r}
table4b |> 
  pivot_longer(c(`1999`, `2000`), 
               names_to = "year", 
...
Now we have two separate data sets containing  `cases` and `population` variables. How can we combine these two tidy data sets? 
...
```{r}
pivot_longer(dfwide, 
             c(`GDPpc2005`, `GDPpc2010`, `GDPpc2015`), 
             names_to = "year", 
             values_to = "GDPpc", 
             names_prefix = "GDPpc"
             )
...
The data set `trdata1`, which we created previously, contains several variables. 
```{r}
trdata1
```
We can handle this case as follows: 
```{r}
trdata1_long <- pivot_longer(trdata1, 
                             c(`y2005`, `y2010`, `y2015`), 
                             names_to = "year", 
                             values_to = "values", 
                             names_prefix = "y")
...
## `pivot_wider()`: Convert to wide form
...
```{r}
trdata1_tidy <- pivot_wider(trdata1_long, 
                            names_from = variable, 
                            values_from = values)
trdata1_tidy
```
...
**Example**: For low-income European countries, draw a time series graph of life expectancy. Use the following rule: if `gdpPercap<10000 in 2007` then classify as low-income, otherwise classify as high-income.  
```{r}
# create the data set
europe <- gapminder |> 
  filter(continent == "Europe") 
clist <- filter(europe, gdpPercap<10000 & year==2007) |> pull(country)
lowincome_europe <- filter(europe, country %in% clist)
# plot 
...
Histogram is useful in visualizing how a continuous variable is distributed. We use  `geom_histogram()` for this. For example: 
...
The most important choice in histogram is the number of bins. Users should select the appropriate number of bins instead of relying on default values. Redraw the histogram with bins=10
```{r} 
y2007 |> ggplot() +
...
```
We see that life expectancy is bimodal in 2007. There seems to be two groups of countries with centers around 55 years and 75 years.   

**Example**: Draw the histogram using all years: 
```{r}
library(gapminder)
ggplot(data =gapminder ) +
  geom_histogram(mapping = aes(x = lifeExp), binwidth = 0.5, color="white")
```
...
class <-  factor( 
  rep(c("Econometrics-1","Econometrics-2","Machine-Learning"), each=n) 
  )
...
Draw the histogram of each class side by side: 
```{r}
...
**Example**: Draw the box plots of classes from the previous example: 
```{r, echo=TRUE}
ggplot(data = gradedata) +
  geom_boxplot(mapping = aes(x = class, y=grades))  
```

**Example:** Draw box plots of life expectancy over continents in the Gapminder data set: 
```{r} 
ggplot(data = gapminder, 
...
ggplot(europe2007, aes(x = lifeExp, y = fct_reorder(country, lifeExp))) +
  geom_point() +
  labs(title = "Life expectancy in Europe (2007)",
       y = "", 
...
trpop_overall <- city_pop[1,]   # TR total population saved in 
trpop_province <- city_pop[-1,] # erase the first row
# rename the first column from "...1" to "province"
trpop_province <- rename(trpop_province, "province"="...1")
trpop_province |> head(5)
```
This table is in wide form as we have years in the columns. We need to create a new variable year, and put values into population: 
```{r}
# wide to long
population <- trpop_province |> 
  pivot_longer(-province,               # all columns except province
               names_to = "year", 
               values_to = "population")
# show the first 5 obs
population |> 
...
# Application: Popular babynames

## Read the data set

Turkish Statistical Institute publishes the 100 most popular men and women's names given to newly born babies in a year. I downloaded the data for men's names in  `xls` format and saved it in the Data folder. Upon inspection of the excel file, we see that data is within the range `A6:BA88` including the variable names. We will not alter the original raw data using excel but make all necessary adjustments using `R`. Let us read it into R: 

```{r}
library(tidyverse)
library(readxl)
mennames2018 <- read_excel("../Data/erkek_isim_siralama2018.xls", 
                           range = "A4:AQ301")
# View(mennames2018) 
```
